...
h.each((function(k){if(!g[k]){if(Ext.isIE){g[k]=new Ext.tree.TreeNode({text:ORYX.I18N.propertyNames[k],allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true,expanded:true});
g[k].expand()
}else{g[k]=new Ext.tree.TreeNode({text:ORYX.I18N.propertyNames[k],allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true})
}b.appendChild(g[k]);
g[k].render()
...
},createStencilTreeNode:function(b,c){var a=c.id().split("#");
var e=new Ext.tree.TreeNode({text:ORYX.I18N.propertyNames[a[1]],icon:decodeURIComponent(c.icon()),allowDrag:false,allowDrop:false,iconCls:"ShapeRepEntreeImg",cls:"ShapeRepEntree"});
b.appendChild(e);
e.render();
var d=e.getUI();
d.elNode.setAttributeNS(null,"title",ORYX.I18N.propertyNames[a[1]+"_desc"]);
Ext.dd.Registry.register(d.elNode,{node:d.node,handles:[d.elNode,d.textNode].concat($A(d.elNode.childNodes)),isHandle:false,type:c.id(),title:c.title(),namespace:c.namespace()})
...
}if(i.value()=="Business Rule"||i.value()=="Script"||i.value()=="None"){y.push([i.icon(),ORYX.I18N.propertyNamesTaskType[i.title()],i.value()]);
X.push({name:ORYX.I18N.propertyNamesTaskType[i.title()],icon:i.icon()})
}})
}else{v.each(function(j){if(j.value()==C){C=j.title()
}if(j.refToView()[0]){G=true
}var i=ORYX.I18N.propertyNamesValue[j.title()];
if(!i){i=j.title()
}y.push([j.icon(),i,j.value()]);
X.push({name:i,icon:j.icon()})
})
...
N=new Ext.Editor(E);
break;
case ORYX.CONFIG.TYPE_CUSTOM:var E=new Ext.form.ComplexCustomField({allowBlank:p.optional(),dataSource:this.dataSource,grid:this.grid,row:F,facade:this.facade,title:ORYX.I18N.propertyNames[p.id()],attr:C});
E.on("dialogClosed",this.dialogClosed,{scope:this,row:F,col:1,field:E});
N=new Ext.Editor(E);
break;
...
}if(p.extra()){this.properties.push([ORYX.I18N.PropertyWindow.moreProps,ORYX.I18N.propertyNames[p.id()],C,X,{editor:N,propId:t,type:p.type(),tooltip:ORYX.I18N.propertyNames[p.id()+"_desc"],renderer:K,labelProvider:this.getLabelProvider(p)}])
}else{if(p.simulation()){this.simulationProperties.push([ORYX.I18N.PropertyWindow.simulationProps,ORYX.I18N.propertyNames[p.id()],C,X,{editor:N,propId:t,type:p.type(),tooltip:ORYX.I18N.propertyNames[p.id()+"_desc"],renderer:K,labelProvider:this.getLabelProvider(p)}])
}else{if(p.display()){this.displayProperties.push([ORYX.I18N.PropertyWindow.displayProps,ORYX.I18N.propertyNames[p.id()],C,X,{editor:N,propId:t,type:p.type(),tooltip:ORYX.I18N.propertyNames[p.id()+"_desc"],renderer:K,labelProvider:this.getLabelProvider(p)}])
}else{this.popularProperties.push([ORYX.I18N.PropertyWindow.oftenUsed,ORYX.I18N.propertyNames[p.id()],C,X,{editor:N,propId:t,type:p.type(),tooltip:ORYX.I18N.propertyNames[p.id()+"_desc"],renderer:K,labelProvider:this.getLabelProvider(p)}])
}}}}}}).bind(this))
}this.setProperties()
